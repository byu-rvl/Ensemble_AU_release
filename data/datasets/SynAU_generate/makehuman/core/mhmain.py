    ...
    def startupSequence(self):
        ...
        if (not self.args.get('noshaders', False) or G.preStartupSettings["noShaders"]) and \
          ( not mh.Shader.supported() or mh.Shader.glslVersion() < (1,20) ):
            ...
            log.message('Warning' + 'GLSL shaders have been disabled or are not supported.\nOnly simple shading will be available.' + 'Ok' + str(None) + str(None) + str(None) + 'glslWarning')
        ...
        if GENERATE_ON_STARTUP:
            self.switchCategory(CATEGORY_MODELING)
            self.switchTask(TASK_FACS_HUMAN)
            self.currentTask.makeAllVideos()

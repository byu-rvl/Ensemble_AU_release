...
#            self.facs_human.updateSubdivisionMesh(True)
        ...
        #put pants and shirt on at start up.

        # #change to the extra add-on tab.
        self.app.switchCategory(CATEGORY_EXTRA_ADD_ON)
        # add clothes here.
        self.app.switchTask(TASK_CLOTHES)
        self.app.currentTask.resetSelection()  # reset current Task selection
        self.addItem(JEAN_SHORTS_FILE, CLOTHES_PXY_TYPE)
        self.addItem(CARGO_PANTS_FILE, CLOTHES_PXY_TYPE)
        self.addItem(POLO_SHIRT_FILE, CLOTHES_PXY_TYPE)
...
##########################################################################
    ...
    def makeOneFrame(self, keys, intensities):
        print("look here now", keys)
        print("look here now1", intensities)
        self.resetFacialCodes()  # reset all sliders.
        ...
        #convert 'PyQt5.QtGui.QImage' to opencv format
        incomingImage = img.convertToFormat(4)
        width = incomingImage.width()
        height = incomingImage.height()
        ptr = incomingImage.bits()
        ptr.setsize(incomingImage.byteCount())
        cvImage = np.array(ptr).reshape(height, width, 4)  # Copies the data
    ...
    def rvl_resetCamera(self):
        ...
        pos = [0, 0.2, 0]
        self.facs_human.setPosition(pos)
        self.camera_slider_x.setValue(random.sample(CAMERA_X_POSSIBLE, 1)[0])
        self.camera_slider_y.setValue(random.sample(CAMERA_Y_POSSIBLE, 1)[0])
        self.camera_slider_zoom.setValue(random.sample(CAMERA_ZOOM_POSSIBLE, 1)[0])
        self.rotation_slider_z.setValue(random.sample(CAMERA_Z_POSSIBLE, 1)[0])
        self.rotation_slider_z.onChanging(0)
        self.rotation_slider_z.update()
        mh.redraw()
        gui3d.app.statusPersist('Camera updated')
    ...
    def makeSingleVideo(self, movementsInVideo, thisSaveDir, personIndx):
        ...
        # make vid directory:
        vidDir = str(movementsInVideo) #crucial this is after sort above.
        vidDir = vidDir.replace("[", "AU_")
        vidDir = vidDir.replace("]", "")
        vidDir = vidDir.replace(",", "_")
        vidDir = vidDir.replace("'", "")
        vidDir = vidDir.replace(" ", "")
        thisSaveDir = thisSaveDir / vidDir
        thisSaveDir.mkdir()

        for num_vid in range(0, NUMBER_VIDEOS_PER_MOTION):
            #compute how big each movement in video will be:
            intensities_max = []
            for movement in movementsInVideo:
                intensities_max.append(random.uniform(MIN_INTENSITY, MAX_INTENSITY))

            haveBlink = random.randint(0, 1)
            if haveBlink:
                blink_length = random.randint(MIN_BLINK_FRAMES, MAX_BLINK_FRAMES)
                blink_start = random.randint(0, NUMBER_FRAMES) #allow blink to start anywhere in video.
                blink_max = blink_start + int(blink_length/2)
                blink_end = blink_start + blink_length
                halfLength = int(blink_length / 2)

            # make video writer
            intensity_string = str(intensities_max)
            intensity_string = intensity_string.replace("[", "")
            intensity_string = intensity_string.replace("]", "")
            intensity_string = intensity_string.replace(",", "_")
            intensity_string = intensity_string.replace("'", "")
            intensity_string = intensity_string.replace(" ", "")
            file_name = "VidNum_" + str(num_vid) + "__intensity" + intensity_string + ".png"
            file_name = str(thisSaveDir/ file_name)

            theseMovements = movementsInVideo.copy()
            theseIntensities = intensities_max.copy()
            frame = self.makeOneFrame(theseMovements, theseIntensities) #make blank frame
    ...
    def makeOneVideo(self, movementsInVideo, thisSaveDir, personIndx):
        ...
        for num_vid in range(0, NUMBER_VIDEOS_PER_MOTION):
            ...
            #get when to start movement.
            blank_until_frame = random.randint(START_MOVEMENT_EARLIEST, START_MOVEMENT_LATEST)

            #get when to have movement done/gone up and come back down. Earliest can be done is the frame started plus up and then plus down.
            end_frame_blank = random.randint(blank_until_frame + UP_MOVEMENT_MIN_FRAMES + DOWN_MOVEMENT_MIN_FRAMES, END_MOVEMENT_BY)

            #get when to have movement up to the up movement.
            up_completed = random.randint(blank_until_frame + UP_MOVEMENT_MIN_FRAMES, end_frame_blank - DOWN_MOVEMENT_MIN_FRAMES)

            #get when to start moving back to blank face
            start_going_down = random.randint(up_completed, end_frame_blank - DOWN_MOVEMENT_MIN_FRAMES)

            lengthUp = up_completed - blank_until_frame
            lengthDown = end_frame_blank - start_going_down
            ...
            intensity_string = intensity_string.replace("'", "")
            intensity_string = intensity_string.replace(" ", "")
            file_name = "VidNum_" + str(num_vid) + "__" + intensity_string + "__" + str(blank_until_frame) + "_" + str(end_frame_blank) + "_" + str(up_completed) + "_" + str(start_going_down) + ".avi"
            file_name = str(thisSaveDir/ file_name)
            fourcc = cv2.VideoWriter_fourcc(*'XVID')
            writer = cv2.VideoWriter(file_name, fourcc, FPS, (int(self.renderingWidth), int(self.renderingHeight)))

            #generate blank frame and max move frame to use when repeat
            for i in range(0, NUMBER_FRAMES):
                if i < blank_until_frame:
                    theseIntensities = []
                    theseMovements = []
                elif i < up_completed:
                    intensityProportion = (i - blank_until_frame) / lengthUp
                    theseIntensities = [i * intensityProportion for i in intensities_max]
                    theseMovements = movementsInVideo.copy()
                elif i < start_going_down:
                    theseIntensities = [1.0]
                    theseMovements = movementsInVideo.copy()
                elif i < end_frame_blank:
                    intensityProportion = (lengthDown - (i - start_going_down)) / lengthDown
                    theseIntensities = [i * intensityProportion for i in intensities_max]
                    theseMovements = movementsInVideo.copy()
                else:
                    theseIntensities = []
                ...
                if haveBlink:
                    if i >= blink_start and i < blink_max:
                        # eye is closing here.
                        blink_intensity = (i - blink_start) / halfLength
                        theseIntensities.append(float(blink_intensity))
                        theseMovements.append(BLINK_AU)
                    elif i == blink_max:
                        blink_intensity = 1.0
                        theseIntensities.append(float(blink_intensity))
                        theseMovements.append(BLINK_AU)
                    elif i >= blink_max and i <= blink_end:
                        blink_intensity = (halfLength - (i - blink_max)) / halfLength
                        theseIntensities.append(float(blink_intensity))
                        theseMovements.append(BLINK_AU)
                ...
                writer.write(frame[:,:,:3]) #the default dimensions here are 500x500. But, there are 4 dimensions...cut the 4th off.
            writer.release()

    def makeAllVideos(self):
        #get the saveDir
        # saveDir = str(datetime.datetime.now())
        # saveDir = "MH_generation_" + saveDir
        # saveDir = saveDir.replace(" ", "__")
        # saveDir = saveDir.replace("-", "_")
        # saveDir = saveDir.replace(":", "_")
        # saveDir = saveDir.replace(".", "_")
        # saveDir = Path(SAVE_LOCATION) / saveDir
        # saveDir.mkdir()
        saveDir = Path(SAVE_LOCATION)
        saveDir.mkdir(exist_ok=True)

        #get all the AU
        allKeys = list(self.sliders.keys())

        #get only good AU
        allKeys_good = []
        for key in allKeys:
            if key not in DO_NOT_USE_AU:
                allKeys_good.append(key)

        # get the combinations that we need.
        from itertools import combinations
        allComb = list(combinations(allKeys_good, NUMBER_MOVEMENTS_AT_ONCE))
        if INCLUDE_BLANK_MOVEMENT:
            allComb.append([])
        allComb_inList = []
        for comb in allComb:
            allComb_inList.append(list(comb)) #each index of allComb was a tuple, change it to be a list.


        for i, person in enumerate(USE_PEOPLE):

            thisPerson_saveDir = saveDir / person[PEOPLE_NAME_KEY]
            if not ONLY_ONE_VID_PER_PERSON:
                cntr = 0
                while thisPerson_saveDir.exists():
                    thisPerson_saveDir = saveDir / (person[PEOPLE_NAME_KEY] + "_" +str(cntr))
                    cntr += 1
            if not thisPerson_saveDir.exists():
                self.loadPerson(person)
                thisPerson_saveDir.mkdir()

                for makeComb in allComb_inList:
                    if MAKE_IMAGES_NOT_VIDEOS:
                        self.makeSingleVideo(makeComb, thisPerson_saveDir, i)
                    else:
                        print("MakeComb", makeComb)
                        print("thisPerson_saveDir", thisPerson_saveDir)
                        print("i", i)
                        self.makeOneVideo(makeComb, thisPerson_saveDir, i)

    def addItem(self, file, pxy_type):
        pxy = proxy.loadProxy(self.app.selectedHuman, file, pxy_type)
        mesh, obj = pxy.loadMeshAndObject(self.app.selectedHuman)
        ...
        self.app.currentTask.adaptProxyToHuman(pxy, obj)
        obj.setSubdivided(self.app.selectedHuman.isSubdivided())  # Copy subdivided state of human

        # Add to selection
        self.app.currentTask.selectedProxies.append(pxy)

        self.app.currentTask.filechooser.selectItem(file)

        if self.app.currentTask.descriptionWidget:
            self.app.currentTask.descrLbl.setText(pxy.description)

        self.app.currentTask.proxySelected(pxy)

        self.app.currentTask.signalChange()

    def loadPerson(self, person):
        # Do body size here.
        self.app.selectedHuman.setGender(person[PEOPLE_GENDER_KEY])
        self.app.selectedHuman.setAge(person[PEOPLE_AGE_KEY])
        self.app.selectedHuman.setWeight(person[PEOPLE_WEIGHT_KEY])
        self.app.selectedHuman.setMuscle(person[PEOPLE_MUSCLE_KEY])
        self.app.selectedHuman.setHeight(person[PEOPLE_HEIGHT_KEY])
        self.app.selectedHuman.setBodyProportions(person[PEOPLE_BODY_PROPORTION_KEY])
        self.app.selectedHuman.setAfrican(person[PEOPLE_AFRICAN_KEY])
        self.app.selectedHuman.setAsian(person[PEOPLE_ASIAN_KEY])
        self.app.selectedHuman.setCaucasian(person[PEOPLE_CAUCASIAN_KEY])

        # changes for the face shape here.
        self.app.switchCategory(CATEGORY_MODELING)
        self.app.switchTask(TASK_FACE)
        headInfo = person[PEOPLE_HEAD_TYPE_KEY]
        for info in headInfo:
            sliderIndx = info[0]
            sliderValue = info[1]
            self.app.currentTask.sliders[sliderIndx].setValue(sliderValue)
            self.app.currentTask.sliders[sliderIndx]._onChange()
            self.app.currentTask.sliders[sliderIndx].update()
        ...
        self.app.switchTask(TASK_CLOTHES)
        self.app.currentTask.resetSelection()  # reset current Task selection
        clothes = person[PEOPLE_CLOTHES_KEY]
        facialHair = person[PEOPLE_FACIAL_HAIR_KEY] #facial hair is classified as clothes.
        for cloth in clothes:
            self.addItem(cloth, CLOTHES_PXY_TYPE)
        for scruff in facialHair:
            self.addItem(scruff, CLOTHES_PXY_TYPE)

        # self.app.switchTask(TASK_EYES)
        # #currently don't have any eyes that I want to change in.

        self.app.switchTask(TASK_HAIR)
        self.app.currentTask.resetSelection()  # reset current Task selection
        theHair = person[PEOPLE_HAIR_KEY]
        for aHair in theHair:
            self.addItem(aHair, HAIR_PXY_TYPE)

        self.app.switchTask(TASK_TEETH)
        self.app.currentTask.resetSelection()  # reset current Task selection
        theTeeth = person[PEOPLE_TEETH_KEY]
        for aTeeth in theTeeth:
            self.addItem(aTeeth, TEETH_PXY_TYPE)

        self.app.switchTask(TASK_EYEBROWS)
        self.app.currentTask.resetSelection()  # reset current Task selection
        eyebrows = person[PEOPLE_EYEBROW_KEY]
        for eyebrow in eyebrows:
            self.addItem(eyebrow, EYEBROW_PXY_TYPE)

        self.app.switchTask(TASK_EYELASHES)
        self.app.currentTask.resetSelection()  # reset current Task selection
        eyelashes = person[PEOPLE_EYELASHES_KEY]
        for eyelash in eyelashes:
            self.addItem(eyelash, EYELASHES_PXY_TYPE)

        self.app.switchTask(TASK_TONGUE)
        self.app.currentTask.resetSelection()  # reset current Task selection
        tongues = person[PEOPLE_TONGUE_KEY]
        for tongue in tongues:
            self.addItem(tongue, TONGUE_PXY_TYPE)

        #KEEP THIS LAST. GOES BACK TO WHERE WE NEED IT!!!!
        self.app.switchCategory(CATEGORY_MODELING)
        self.app.switchTask(TASK_FACS_HUMAN)

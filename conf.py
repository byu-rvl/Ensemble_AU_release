"""
Code originally from https://github.com/CVI-SZU/ME-GraphAU file: model/conf.py
Modified and adapted to work in the ELEGANT framework by Andrew Sumsion
Modified and adapted to work in the Ensemble framework by Andrew Sumsion
"""
...
def set_outdir(conf):
    ...
    if conf.exp_name == 'use_resume':
        resume = Path(conf.resume)
        conf.exp_name = resume.parent.parent.name
        dir_name = conf.exp_name
    else:
        timestr = datetime.now().strftime('%d-%m-%Y_%I_%M-%S_%p')
        dir_name = conf.exp_name + '_' + timestr
    outdir = os.path.join(default_outdir,dir_name)
